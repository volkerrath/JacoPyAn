...
for using Jacobians generated by _ModEM_ [3, 4]. As of today, the routines for manipulating/sparsifying the full Jacobian, the SVD, and the calculation of sensitivity 
can be used.
...
action, a  few steps are necessary. _ModEM_ seeks the MAP solution to the usual Bayesian inverse problem [5] defined by:
...
```
The jJacobian used within _ModEM_ is also calculated in the transformed system:
